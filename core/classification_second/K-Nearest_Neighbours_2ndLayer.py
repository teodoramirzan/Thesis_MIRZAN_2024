...
if dangerous_data.empty:
    ...
    # Creează și antrenează modelul KNN
    model_knn_layer2 = KNeighborsClassifier(n_neighbors=5)
    model_knn_layer2.fit(X_train, y_train)

    # Salvează modelul antrenat într-un fișier
    model_path_layer2 = "C:\\Users\\Matebook 14s\\Desktop\\Licenta\\saved_models\\knn_model_2nd.joblib"
    joblib.dump(model_knn_layer2, model_path_layer2)

    # Evaluează modelul
    y_pred_layer2 = model_knn_layer2.predict(X_test)
    ...
    print(f"Acuratețea modelului pentru nivelul 2: {model_knn_layer2.score(X_test, y_test) * 100:.2f}%")

    print("Modelul KNN pentru nivelul 2 a fost antrenat și salvat la:", model_path_layer2)
